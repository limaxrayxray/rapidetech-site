...
- **`faq`** (collection, créée) : `question`, `answer`, `sort`. Alimente le JSON-LD FAQPage de l'accueil via `getFaq()` (invisible — pas de rendu HTML visible, seulement le `<script type="application/ld+json">` dans `Base.astro`). Permission `read` sur la policy « Build — lecture seule » en place.
...
`ContactForm.astro` POST vers `/api/lead` (Nginx → `deploy/lead-mailer.mjs`, port local 8788 par défaut — ⚠️ **sur CE VPS le port est 8791** via `LEAD_PORT` : 8788 était déjà pris par un service invisible d'un autre user ; le vhost CloudPanel pointe donc sur 8791, avec `proxy_set_header X-Forwarded-For $remote_addr;` (sinon le rate-limit voit tout le monde en 127.0.0.1)). ⚠️ **Pas de root sur le VPS** (CloudPanel, user `galx-dev-ssh`, clé root indisponible) → le relais tourne SANS systemd : `deploy.sh` pousse le code vers `~/apps/rapidetech-leads/` et le crontab de l'utilisateur supervise via `lead-mailer-watchdog.sh` (`@reboot` + `*/5min`, pgrep avant relance). Node installé SANS root (binaire officiel dans `~/.local/node`). Config dans `~/rapidetech-leads.env` (PAS /etc, jamais écrasée par les déploiements), journal dans le HOME. La directive Nginx s'ajoute par l'admin web CloudPanel (Sites → Vhost Editor), jamais par /etc. Le unit systemd `rapidetech-leads.service` reste dans le repo si root devient disponible un jour. Envoi via l'API HTTP de SMTP2GO (clé dans `/etc/rapidetech-leads.env`, jamais dans le repo ; le succès se vérifie sur `data.succeeded ≥ 1`, pas juste le 200). Honeypot `entreprise`, rate-limit 5/h/IP, retries, et journal JSONL de secours (lead jamais perdu même si SMTP2GO tombe). Sans JS : POST classique → 303 vers `/merci/`. ⚠️ Le formulaire porte `data-astro-reload` + listener submit en phase CAPTURE — sinon le ClientRouter d'Astro intercepte le submit et « navigue » vers /api/lead.
**Anti-spam Turnstile (Cloudflare, root du domaine SEULEMENT)** : le widget ne se rend que si `PUBLIC_TURNSTILE_SITE_KEY` est dans le `.env` de build (→ build prod du root uniquement ; préprod/hosts locaux : laisser vide → aucun changement pour eux). Côté relais, `TURNSTILE_SECRET` (dans `~/rapidetech-leads.env`) active la vérif siteverify, et `TURNSTILE_HOSTNAMES` (ex. `rapidetech.ca,www.rapidetech.ca`) rejette tout jeton résolu sur un autre host. Variables vides = désactivé (comportement d'avant). siteverify injoignable = fail-open (pas de lead perdu sur une panne Cloudflare). ⚠️ Secret actif ⇒ une soumission no-JS n'a pas de jeton → 403 (compromis anti-spam assumé). Jeton à usage unique : le script du formulaire fait `turnstile.reset()` après chaque envoi fetch et re-rend le widget après une navigation ClientRouter. Les clés ne vont JAMAIS dans Directus ni dans le repo : site key (publique) dans le `.env` de la machine de build, secret dans `~/rapidetech-leads.env` sur le VPS.

### Avis Google → Directus (sync automatique)
`deploy/sync-google-reviews.mjs` (cron sur la machine Directus) : API Places (New) → met à jour `site_settings.google_rating`/`google_reviews_count` SEULEMENT. **Les témoignages affichés sont saisis à la main dans Directus** (choix client : l'API publique est limitée à ~5 avis « pertinents », sa sélection parmi ses 44+ avis est plus riche pour la rotation). L'import auto des ~5 avis reste disponible via `GOOGLE_IMPORT_REVIEWS=1` (upsert dédup. par `google_review_id` ; ne touche jamais les témoignages manuels). Les témoignages affichés sont mélangés à CHAQUE BUILD **et re-mélangés à CHAQUE VISITE** : toutes les cartes sont rendues dans le HTML (celles au-delà de 6 avec `hidden`), un script inline d'`index.astro` les re-mélange/renumérote au chargement (no-JS safe : l'ordre du build reste).
...
- **robots.txt dynamique** (`src/pages/robots.txt.ts`, PAS un fichier `public/`) : en prod, allow all + directives explicites pour les crawlers IA (GPTBot, OAI-SearchBot, ClaudeBot, PerplexityBot, Bingbot — GEO : ChatGPT search lit l'index Bing) + référence sitemap. Ne jamais créer `public/robots.txt` : ça entrerait en conflit avec la route et perdrait la protection préprod.
- Sitemap (`@astrojs/sitemap`, `/merci/` et `/404` exclus), canonical, Open Graph (`public/og.png`), favicon.svg.
- **Meta head = composant `SEO.astro`** (monté dans `Base.astro`) : title, description, canonical, OG complet, twitter:card. Chaîne de fallback : props de page → `site_settings.meta_title`/`meta_description` (Directus, fillEmpty) → `FALLBACK_SITE_SETTINGS`.
- **JSON-LD = composant `SchemaOrg.astro`** (monté dans `Base.astro`, donc TOUTES les pages) : ProfessionalService (nom, description, url, téléphone/courriel — omis si vides dans Directus, jamais de placeholder publié —, adresse, `areaServed` = `site_settings.cities`, AggregateRating seulement si `google_reviews_count` est renseigné). FAQPage émis sur l'accueil depuis la collection Directus `faq` (question/réponse/sort — `getFaq()` dégrade en `[]` tant que la collection n'existe pas) et sur les pages services depuis leur champ `faq`.
- **IndexNow** (Bing → ChatGPT search ; Google l'ignore) : clé publique `df2b02d5d2c5471cc229a10970e667f6` servie via `public/<clé>.txt`. `deploy.sh` extrait les URLs du sitemap après le rsync et les soumet en un seul POST à `api.indexnow.org` — jamais bloquant, code HTTP loggé. Test à sec : `INDEXNOW_DRY_RUN=1`. Si la clé change : régénérer (`openssl rand -hex 16`), remplacer le fichier `public/` ET `INDEXNOW_KEY` dans `deploy.sh`.
...
2. **Flow Directus** « Déploiement auto du site » : trigger event (create/update/delete) sur les collections de contenu (`home`, `services`, `differentiators`, `about`, `testimonials`, `partners`, `site_settings`, `privacy`, `faq`) → opération *request* qui POST `http://172.18.0.1:8787/deploy` avec le header `Authorization: Bearer <secret>`. ⚠️ Toute nouvelle collection de contenu lue au build doit être ajoutée à cette liste, sinon un edit ne déclenche pas de rebuild automatique.
