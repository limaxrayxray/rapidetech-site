...
- **`faq`** (collection — ⚠️ PAS ENCORE CRÉÉE dans Directus) : `question`, `answer`, `sort`. Alimente le JSON-LD FAQPage de l'accueil via `getFaq()` (fallback silencieux `[]` tant qu'elle n'existe pas). À la création : suivre les pièges ci-dessus (PK uuid `special:["uuid"]`, `display_template`, `sort`) ET ajouter la permission `read` à la policy « Build — lecture seule », sinon fallback silencieux.
